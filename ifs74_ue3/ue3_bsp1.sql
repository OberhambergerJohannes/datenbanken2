...
CREATE OR REPLACE TYPE Gebäude_REF_NESTED_TABLE_TYPE AS TABLE OF REF Gebäude_Typ;
/

ALTER TYPE Grundstücke_Typ
-- ADD ATTRIBUTE (GebäudeNested Gebäude_NESTED_TABLE_TYPE) 
ADD ATTRIBUTE (GebäudeRefNested Gebäude_REF_NESTED_TABLE_TYPE)	--this would be a reference to Gebäudetyp ??
CASCADE;
/
...
BEGIN
    ...
    INSERT INTO Immobilien_Tab VALUES (
        WALDGRUNDSTÜCKE_TYP(
            ...
            7000,
            'coole Lage',
            Gebäude_REF_NESTED_TABLE_TYPE(gebäude_ref),
            'Tannenwald'
        )
    );
END;
